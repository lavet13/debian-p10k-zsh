...
# Assumes the CachyOS KDE Plasma edition as the base — provides Plasma, plasma-apply-colorscheme, kcmshell6.
#
# ----------------------------------------------------------------------------
...
# the system dark-mode; kde-gtk-config + breeze-gtk generate the GTK colors.css
# from the KDE scheme; plasma-integration = the `kde` Qt platform theme;
# xdg-desktop-portal-gtk = the color-scheme portal broadcast (GTK4/Firefox dark).
sudo pacman -S --needed --noconfirm \
  ...
  trash-cli pavucontrol gnome-themes-extra gsettings-desktop-schemas \
  kde-gtk-config breeze-gtk plasma-integration xdg-desktop-portal-gtk
...
# || true: gsettings needs a session bus; a bare-TTY run would otherwise abort set -e.
...
# GTK app colours (~/.config/gtk-{3,4}.0/colors.css) are GENERATED by kde-gtk-config
# from the applied KDE scheme — NOT tracked here (derived artifact). The sync module
# lives in kded6, which isn't started under sway. To generate once on a fresh install,
# from inside a sway session run:  kded6 & sleep 3 ; pkill -x kded6
# (it writes colors.css on module load; the Plasma-service errors it prints are harmless).
# Fallback: log into KDE once and re-apply Naysayer in System Settings > Colors.
# Retune Naysayer.colors → repeat to regenerate.
